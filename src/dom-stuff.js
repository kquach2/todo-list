  ...
  if (ProjectManager.projectsArray[0].todos.length > 0) {
    for (let i = 0; i < ProjectManager.projectsArray[0].todos.length; i += 1) {
      ...
      todoDueDate.textContent =
        ProjectManager.projectsArray[0].todos[i].dueDate;
...
const displayTodayTodos = () => {
  ...
  for (let i = 0; i < ProjectManager.projectsArray.length; i += 1) {
    if (ProjectManager.projectsArray[i].todos.length > 0) {
      for (
        let j = 0;
        j < ProjectManager.projectsArray[i].todos.length;
        j += 1
      ) {
        const today = new Date(new Date().setHours(0, 0, 0, 0));
        const dueDate = new Date(
          `${ProjectManager.projectsArray[i].todos[j].dueDate}T00:00:00`
        );
        if (differenceInDays(today, dueDate) === 0) {
          ...
          button.classList.add(
            ProjectManager.projectsArray[i].todos[j].priority
          );
          button.addEventListener("click", (event) => {
            event.stopPropagation();
          ...
          todoDueDate.setAttribute("class", "todo-duedate");
          todoDueDate.textContent =
            ProjectManager.projectsArray[i].todos[j].dueDate;
          right.appendChild(todoDueDate);
      ...
      todo.addEventListener("click", () => {
        expandTodo(
          ProjectManager.projectsArray[
            parseInt(currentlyActive.dataset.projectIndex, 10)
          ].todos[i]
        );
      });
      const left = document.createElement("div");
...
const displayUpdateTaskForm = (projectIndex, todoIndex) => {
  ...
  form.addEventListener("submit", () => {
    ...
    const currentlyActive = document.querySelector(".active");
    ProjectManager.projectsArray[projectIndex].updateTodo(
      todoIndex,
      newTitle,
      newDescription,
      newDueDate,
