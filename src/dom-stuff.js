...
const displayInboxTodos = () => {
  ...
  if (ProjectManager.projectsArray[0].todos.length > 0) {
    for (let i = 0; i < ProjectManager.projectsArray[0].todos.length; i += 1) {
...
const displayTodayTodos = () => {
  ...
  const todoContainer = document.createElement('div');
  todoContainer.setAttribute('id', 'todo-container');
  for (let i = 0; i < ProjectManager.projectsArray.length; i += 1) {
    if (ProjectManager.projectsArray[i].todos.length > 0) {
      for (let j = 0; j < ProjectManager.projectsArray[i].todos.length; j += 1) {
        const today = new Date(new Date().setHours(0, 0, 0, 0));
        const dueDate = new Date(`${ProjectManager.projectsArray[i].todos[j].dueDate}T00:00:00`);
        if (differenceInDays(today, dueDate) === 0) {
          ...
          const todoDueDate = document.createElement('p');
          todoDueDate.setAttribute('class', 'todo-duedate');
          todoDueDate.textContent = ProjectManager.projectsArray[i].todos[j].dueDate;
          right.appendChild(todoDueDate);
...
const displayCustomProjectTodos = (project) => {
  ...
  if (project.todos.length > 0) {
    for (let i = 0; i < project.todos.length; i += 1) {
      ...
      todo.addEventListener('click', () => {
        expandTodo(ProjectManager.projectsArray[parseInt(currentlyActive.dataset.projectIndex, 10)].todos[i]);
      });
      const left = document.createElement('div');
      left.setAttribute('class', 'left');
      ...
      const button = document.createElement('button');
      button.setAttribute('class', 'complete-button');
      button.classList.add(ProjectManager.projectsArray[parseInt(currentlyActive.dataset.projectIndex, 10)].todos[i].priority);
      button.addEventListener('click', (event) => {
        event.stopPropagation();
        ProjectManager.projectsArray[parseInt(currentlyActive.dataset.projectIndex, 10)].deleteTodo(i);
      ...
      editIcon.addEventListener('click', (event) => {
        event.stopPropagation();
        displayUpdateTaskForm(parseInt(currentlyActive.dataset.projectIndex, 10), i);
      });
      right.appendChild(editIcon);
      ...
      deleteIcon.addEventListener('click', (event) => {
        event.stopPropagation();
        ProjectManager.projectsArray[parseInt(currentlyActive.dataset.projectIndex, 10)].deleteTodo(i);
        displayCustomProjectTodos(ProjectManager.projectsArray[parseInt(currentlyActive.dataset.projectIndex, 10)]);
      });
...
const displayNewTaskForm = () => {
  ...
  form.addEventListener('submit', () => {
    ...
    if (currentlyActive.id === 'inbox-project') {
      ...
      ProjectManager.projectsArray[parseInt(currentlyActive.dataset.projectIndex, 10)].addTodo(newTodo);
      displayCustomProjectTodos(ProjectManager.projectsArray[parseInt(currentlyActive.dataset.projectIndex, 10)]);
    }
  });
...
const displayUpdateTaskForm = (projectIndex, todoIndex) => {
  ...
  form.addEventListener('submit', () => {
    ...
    const currentlyActive = document.querySelector('.active');
    ProjectManager.projectsArray[projectIndex].updateTodo(todoIndex, newTitle, newDescription, newDueDate, newPriority);
    if (currentlyActive.id === 'inbox-project') {
      displayInboxTodos();
    } else if (currentlyActive.id === 'today-project') {
      displayTodayTodos();
      displayThisWeekTodos();
    } else displayCustomProjectTodos(ProjectManager.projectsArray[parseInt(currentlyActive.dataset.projectIndex, 10)]);
  });
  ...
  const todo = ProjectManager.projectsArray[projectIndex].todos[todoIndex];
  const taskName = document.createElement('div');
  const taskNameLabel = document.createElement('label');
  taskNameLabel.setAttribute('for', 'taskName');
  taskNameLabel.textContent = 'Task name:';
...
const displayProjects = () => {
  ...
  if (ProjectManager.projectsArray.length > 1) {
    for (let i = 1; i < ProjectManager.projectsArray.length; i += 1) {
      ...
      deleteIcon.addEventListener('click', (event) => {
        ...
        if (document.querySelector('.active')) {
          previouslyActiveIndex = parseInt(document.querySelector('.active').dataset.projectIndex, 10);
        }
        displayProjects();
        const projectPreview = document.querySelector('#project-preview');
...
const displayNewProjectForm = () => {
  ...
  form.addEventListener('submit', () => {
    ...
    let previouslyActiveIndex = -1;
    if (document.querySelector('.active')) previouslyActiveIndex = parseInt(document.querySelector('.active').dataset.projectIndex, 10);
    displayProjects();
...
export {
  clearContainer, displayInboxTodos, displayTodayTodos, displayThisWeekTodos,
  displayCustomProjectTodos, displayProjects, expandTodo, displayNewTaskForm,
  displayUpdateTaskForm, displayNewProjectForm,
};
